#include "BranchAndBoundBFSSymmetric.h"
#include "SaveToCSV.h"
...
// Konstruktor klasy BranchAndBoundBFSSymmetric
BranchAndBoundBFSSymmetric::BranchAndBoundBFSSymmetric(const int** input_matrix, int num_cities_input)
...
// Destruktor klasy BranchAndBoundBFSSymmetric
BranchAndBoundBFSSymmetric::~BranchAndBoundBFSSymmetric() {
    for (int i = 0; i < num_cities; ++i) {
        delete[] matrix[i];
    }
    delete[] matrix;
...
void BranchAndBoundBFSSymmetric::preprocessMinEdges() {
    for (int i = 0; i < num_cities; ++i) {
...
BranchAndBoundBFSSymmetric::Result BranchAndBoundBFSSymmetric::solve() {
        ...
        for (int i = current.path[current.path_length - 1] + 1; i < num_cities; ++i) {
            if (!Utilities::isCityInPath(current.path, current.path_length, i)) {
                ...
                if (child_bound < min_cost) {
                    q.enqueue({child_path, current.path_length + 1, temp_cost, child_bound});
                } else {
                    delete[] child_path;
                }
            }
    ...
    // Zapis do pliku
    SaveToCSV save("BranchAndBoundBFSSymmetricResults.csv");
    save.saveResults("BFS_SYM", seconds, milliseconds, nanoseconds, best_path, best_path_length, min_cost);
